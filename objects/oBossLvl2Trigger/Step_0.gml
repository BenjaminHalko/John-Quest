...
if (active == 1 and audio_sound_get_gain(oLvl2Controller.music) == 0) {
	...
	audio_play_sound(snExplosion, 1, false, 0.5);
	...
	instance_create_layer(x+2,y-2,"Instances",oExplode);
	instance_create_layer(x+30,y-2,"Instances",oExplode);
	instance_create_layer(x+16,y+6,"Instances",oExplode);
	active = 2;
}

if (active == 2 and !audio_is_playing(snExplosion) and --wait <= 0) {
	active = 3;
	oLvl2Controller.music = audio_play_sound(mLvl2MusicBoss,1,true);
	audio_sound_loop_start(oLvl2Controller.music, 60 / 140 * 8 * 4);
}

if (active == 3 and !audio_is_playing(snBossLvl1Roar)) {
