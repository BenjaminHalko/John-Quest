...
for(var i = 0; i < array_length(credits); i++) {
	...
	if (array_length(_text) == 1) {
        ...
        if (is_numeric(_text[0])) {
            draw_text_ext(_x,_y + global.fontRetroYOffset,$"~ {worldText}   ~",global.fontRPGSpacing,500);
            draw_sprite(sWorld,_text[0],_x-string_width($"~ {worldText} 0 ~")/2+string_width($"~ {worldText} "),_y);
        } else {
            draw_text_ext(_x,_y + global.fontRetroYOffset,string_upper(_text[0]),global.fontRPGSpacing,500);
        }
        ...
        if (struct_exists(languageFonts, _text[0])) {
            ...
            draw_text_ext(_x,_y + _font.fontRetroYOffset,_text[0],global.fontRPGSpacing,500);
        } else {
            draw_set_font(global.fontRPG);
            draw_text_ext(_x,_y + global.fontRetroYOffset,_text[0],global.fontRPGSpacing,500);
        }
